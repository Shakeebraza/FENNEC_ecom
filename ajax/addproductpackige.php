...
if (empty($_POST['boostPlan'])) {
    $errors['boostPlan'] = 'boostPlan is required.';
}else{
    $planId=$_POST['boostPlan'];
}
...
try {
    ...
    $planData = $dbFunctions->getDatanotenc('boost_plans', "id = '$planId'");
    if (!empty($planData)) {
        $boostType = $planData[0]['slug'];
        $planId = $planData[0]['id'];
        $price = $planData[0]['price'];
        $planName = $planData[0]['name'];
    
        // Return success response with data for redirection
        echo json_encode([
            'success' => true,
